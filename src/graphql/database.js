...
export default class Database {
  ...
  getConnectors() {
    const { amazonS3, uploads, staffProperties } = this.settings.publicSettings
    ...
    return {
        ...
        return Post.findAll({
          ...
          return staffs.map(staff => {
            ...
            staff[prefix + 'postmeta'].map(staffmeta => {
              if(staffProperties.includes(staffmeta.meta_key))
                staffProperty[staffmeta.meta_key] = staffmeta.meta_value
            })
      ...
      getPostsInCategory(termId, { post_type, limit = 10, skip = 0 }) {
        return TermRelationships.findAll({
          ...
          limit: limit,
          offset: skip
        }).then(posts => _.map(posts, post => post[prefix+'post']))
      },
